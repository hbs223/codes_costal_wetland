...
Machine learning model construction for features without land use change.
"""
...
grid_search = GridSearchCV(estimator = pipe_xgb,
                           ...
                           scoring = scoring,     # 既输出mse，又输出r2
                           cv=kfold,
                        #    verbose=1,
                           verbose=2,   # 数值越高，打印台输出信息越详细  便于查看网格搜索的进度
                           n_jobs=-1 , 
                           refit='accuracy',  # 使用accuracy来选择最终的最佳参数
                           )
...
# save model
model_name = r'E:\Desktop\滨海湿地生态质量评估指标与任务进程\论文中的代码整理上传github\7_Xgboost\输出模型\中国滨海湿地影响因素建模_XGB_不带土地利用.joblib'
