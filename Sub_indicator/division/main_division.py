...
This script is used to calculate division.
"""
...
if __name__ == '__main__':
    ...
    with tqdm(total=block_list[0] * block_list[1] , desc="Processing", unit="iteration") as pbar:
        for row in range(0 , im_height , block_size[0]):       
            for col in range(0 , im_width , block_size[1]):  
                ...
                # multiprocessing
                sub_im_arrays_blocks = Parallel(n_jobs = n_jobs , batch_size = batch_size)(delayed(sliding_window)(sub_arr , window_num_list) for sub_arr in blocks)
    ...
    convert_arr_from_float2int(sub_im_arrays , enlargement_factor = 10000 , convert_type = convert_type) 
    ...
    #####################################################################################
    name = r"E:\Desktop\中国滨海湿地数据集\output\中国滨海湿地_division_{}.tif".format(year)
    mat = final_data
    # array_to_tif(name , mat , im_geotrans ,im_proj)
