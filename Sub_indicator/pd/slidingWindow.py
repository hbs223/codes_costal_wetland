...
def sliding_window(arr , window_num_list , res):
    ...
    if (arr.shape[0] >= window_num_list[0]) and (arr.shape[1] >= window_num_list[1]):
        ...
        shannon_index_2D = np.apply_along_axis(custom_operation , axis = 0 , arr = window_view , window_num_list = window_num_list , res = res)
